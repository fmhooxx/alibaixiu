...
$.ajax({
    ...
    {{/each}}
    `;
		var html = template.render(randomTpl, { result: data });
		// console.log(html);
		$('#randomBox').html(html);
	}
});
...
// 向服务器端发送请求 索要最新评论数据 因为没有最新评论的数据 所以就不要渲染到页面上了
$.ajax({
	type: 'get',
	url: '/comments/lasted',
	success: function (data) {
		var commentTpl = `
			...
			{{/each}}
		`;
		var html = template.render(commentTpl, { result: data });
...
$.ajax({
	...
	success: function (data) {
		var navTpl = `
			...
			{{/each}}
		`;
		var html = template.render(navTpl, { result: data });
		$('#navBox').html(html)
		$('#topNavBox').html(html)
	}
});
